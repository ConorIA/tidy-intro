...
xxx Ken The kind of syntax I like is to pipe the data frame in, thus:

```{r, eval=F}
dat %>% rename(Date=..., ...) -> dat
```

I know you like the left-arrow at the beginning, which I'm fine with, but I don't like eg `rename` or `filter` sometimes having a data frame as the first argument and sometimes not. I think that's confusing. 
This happens much of the way through, and *I* think it ought to be changed to do it the way I did it.
...
xxx Ken: ouch! How about these two options below:
...
xxx Ken I'm not sure you need the `rowwise`. If you take it out and use `pmax` (parallel max) instead, it should work fine:

```{r}
dat_win %>% select(Winter, MeanTemp) %>% 
  mutate(HDD=pmax(0, 18 - MeanTemp, na.rm = TRUE))
```

Another way is `ifelse`, or `case_when` that you use later:

```{r}
dat_win %>% select(Winter, MeanTemp) %>% 
  mutate(HDD = ifelse(MeanTemp>=18, 0, 18-MeanTemp))
```

```{r}
dat_win %>% select(Winter, MeanTemp) %>% 
  mutate(HDD = case_when(
    MeanTemp>=18 ~ 0,
    TRUE         ~ 18-MeanTemp))
```

I think one of these makes the logic clearer to the audience.
...
xxx Ken: "grouping rowwise data strips rowwise nature" is a hint to not use `rowwise`, IMO. xxx
...
xxx Ken: to be more `stringr`, use `str_c` instead of `paste0` below. You don't need `library(stringr)` since that's loaded with the `tidyverse`.
Also, the construction of the breaks and labels seems rather complex. xxx
...
```{r, message=FALSE}
demand <- read_csv("https://raw.githubusercontent.com/ConorIA/tidy-intro/master/data/TO_energy_demand.csv")
demand
```
