...
```{r}
dat_long <- dat %>% gather(key = Variable, value = value, -"Date")
```
...
```{r}
dat %>% gather(key = Variable, value = value, -"Date") %>%
...
```{r}
dat_win_sum <- filter(dat_win_sum, n >= 90) %>% select(-n)
...
```{r}
dat_win_demand <- dat_win_demand %>% select(Winter, Label, HDD, Demand = Mean_mean)
dat_win_demand
```
