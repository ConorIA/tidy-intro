...
I talk about some of these today (1-4 and maybe 8),
and later Conor looks at a case study in which some of these tools are brought together.
...
```{r}
thing=read_delim(f, "thing.txt", delim = " ")
```
...
```{r}
covid <- covid %>% select(prov = prname, date, starts_with("num"))
covid
```
...
```{r}
ggplot(covid, aes(x = date, y = numtotal, colour = prov)) + geom_smooth(span = 0.2)
```

```{r}
covid %>% mutate(prov = ifelse(prov %in% c("Ontario", "Quebec"), prov, "RoC")) %>% 
  group_by(prov, date) %>% 
  summarize_all(sum) %>% 
  ggplot(aes(x = date, y = numtotal, colour = prov)) + geom_smooth(span = 0.2)
```
...
In `pivot_longer`:
- which columns do you want to gather up? (eg. with a select-helper)
...
covid %>% mutate(prov = ifelse(prov %in% c("Ontario", "Quebec"), prov, "RoC")) %>% 
  group_by(prov, date) %>% 
  summarize_all(sum) %>% 
  select(prov, date, numtotal, numdeaths) %>% 
...
```{r}
usa1850 %>% pivot_longer(-`State Name`, "birthplace", values_to = "freq") %>% 
...
- [Cheatsheets](https://rstudio.com/resources/cheatsheets/) from RStudio Cloud
- [R for Data Science](https://r4ds.had.co.nz/) - book by creator of Tidyverse, Hadley Wickham 
